...
public class Character extends Pane {
    ...
    public void moveX(int x) {
        // if (x>=Main.SCREEN_WIDTH-32) x = Main.SCREEN_WIDTH-32;
        boolean right = x > 0 ? true : false;
        for (int i = 0; i < Math.abs(x); i++) {
            if (this.getTranslateX() < 0) this.setTranslateX(0);
            if (this.getTranslateX() > Main.SCREEN_WIDTH - 32) this.setTranslateX(Main.SCREEN_WIDTH - 32); // !!!!
    ...
    public void moveY(int y) {
        ...
        for (int i = 0; i < Math.abs(y); i++) {
            if (this.getTranslateY() < 0) this.setTranslateY(0);
            if (this.getTranslateY() > Main.SCREEN_HEIGHT - 32) this.setTranslateY(Main.SCREEN_HEIGHT - 32); // !!!!
